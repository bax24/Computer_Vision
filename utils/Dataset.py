...
import src
from utils.utils import count_peaks_2d, shuffle_lst, heat_map_gen


class Dataset(torch.utils.data.Dataset):

    def __init__(self,
                 dataset_path=src.MAIN_DATASET,
                 train_prop=0.8,
                 train=True,
    ...
    def __getitem__(self, index: int):
        ...
        if '{}.txt'.format(self.dataset[index]) in self.ann_lst:
            tmp = pd.read_csv('{}/Annotations/{}.txt'.format(self.dataset_path, self.dataset[index]), sep=' ',
                              header=None)
...
def nb_cell_histogram():
    ...
    img_lst = os.listdir('{}/Images'.format(src.MAIN_DATASET))
    tot_frame = len(img_lst)

    hist = np.zeros(7)
    # Read each annotation files to construct the histogram
    ann_list = os.listdir('{}/Annotations'.format(src.MAIN_DATASET))
    for itm in ann_list:
        df = pd.read_csv('{}/Annotations/{}'.format(src.MAIN_DATASET, itm), sep=' ', header=None)
        nb_cell = df.shape[0]
        if nb_cell == 0:
            print(df)
        hist[nb_cell] += 1
...
if __name__ == '__main__':
    ...
    dataset = Dataset(data_augmentation=False)
    ...
    for idx in idx_lst:
        ...
        img_, target = dataset.__getitem__(idx)
